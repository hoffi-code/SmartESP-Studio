...
`ses.config.<NAME> = ...` (or patch.object) before exercising a route.
"""
...
SES_MODE = normalize_runtime_mode(os.environ.get("SES_MODE", "addon"))
ESPHOME_IS_HA_ADDON = is_truthy(os.environ.get("ESPHOME_IS_HA_ADDON", "true" if SES_MODE == "addon" else "false"))
SES_STORAGE_MODE = os.environ.get("SES_STORAGE_MODE", "").strip()
SES_VERSION = os.environ.get("SES_VERSION", "").strip()
SES_AUTH_MODE = os.environ.get("SES_AUTH_MODE", "none").strip().lower()
SES_AUTH_USERNAME = os.environ.get("SES_AUTH_USERNAME", "").strip()
SES_AUTH_PASSWORD = os.environ.get("SES_AUTH_PASSWORD", "")
SES_AUTH_PASSWORD_FILE = os.environ.get("SES_AUTH_PASSWORD_FILE", "").strip()
SES_STATUS_USE_PING = is_truthy(os.environ.get("SES_STATUS_USE_PING", "false"))
