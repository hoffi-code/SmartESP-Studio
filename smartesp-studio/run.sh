...
if [ "$mode" != "addon" ] && [ "$mode" != "standalone" ]; then
  echo "[error] Invalid SES_MODE: $mode. Expected addon or standalone." >&2
  exit 1
fi
...
if [ "$mode" = "addon" ]; then
  ...
  port="${SES_PORT:-${PORT:-8099}}"
  use_esphome_shared_path="$(as_bool "${SES_USE_ESPHOME_SHARED_PATH:-${USE_ESPHOME_SHARED_PATH:-false}}")"
...
if [ "$use_esphome_shared_path" = "1" ]; then
  ...
  default_target_dir="/config/smartesp"
  pio_cache_fallback_base="/config/.smartesp/platformio"
  storage_mode="independent_smartesp"
fi

if [ "$mode" = "standalone" ] && [ -n "${SES_TARGET_DIR:-}" ]; then
  target_dir="$SES_TARGET_DIR"
else
...
if [ -n "${SES_PIO_CACHE_DIR:-}" ]; then
  pio_cache_base="$SES_PIO_CACHE_DIR"
elif [ -d /cache ]; then
...
export SES_MODE="$mode"
export SES_STORAGE_MODE="$storage_mode"
export TARGET_DIR="$target_dir"
export PROJECT_DIR="$project_dir"
export PORT="$port"
export ASSET_ROOT="$asset_root"
export SEED_ROOT="$seed_root"
export JOB_DIR="${SES_JOB_DIR:-/data/jobs}"
export ESPHOME_CONFIG_DIR="${SES_ESPHOME_CONFIG_DIR:-/config/esphome}"
export ESPHOME_DATA_DIR="${SES_ESPHOME_DATA_DIR:-/data/esphome}"
export WEB_ROOT="/web"
export PATH="/opt/designer-venv/bin:$PATH"
export ESPHOME_BIN="${SES_ESPHOME_BIN:-esphome}"
...
echo "[info] Starting SmartESP Studio API"
echo "[info] SmartESP Studio version: ${SES_VERSION:-unknown}"
echo "[info] ESPHome: $(esphome version 2>/dev/null || echo unknown)"
echo "[info] Mode: $SES_MODE"
echo "[info] Storage mode: $SES_STORAGE_MODE"
echo "[info] Port: $PORT"
