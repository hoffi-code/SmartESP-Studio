...
Project renamed to **SmartESP Studio** and reset to a 0.x line under [Semantic Versioning](https://semver.org/).

* Renamed from "ESPConfig Designer". Repository, add-on slug, Docker image, and UI branding updated.
* **Breaking:** environment variables are now prefixed `SES_` instead of `ECD_` (`SES_MODE`, `SES_AUTH_*`, `SES_PORT`, …).
* **Breaking:** independent storage path moved from `/config/ecd` to `/config/smartesp`. Move existing data or set `SES_TARGET_DIR`.
* Removed the YouTube and PayPal links from the header.

Everything below this line is the history of the project under its previous name.
