...
SPEC = importlib.util.spec_from_file_location("ses_server", SERVER_PATH)
...
class RuntimeAccessTests(unittest.TestCase):
    def setUp(self):
        self.original_mode = getattr(server, "SES_MODE", "addon")
        self.original_auth_mode = getattr(server, "SES_AUTH_MODE", "none")
        self.original_auth_username = getattr(server, "SES_AUTH_USERNAME", "")
        self.original_auth_password = getattr(server, "SES_AUTH_PASSWORD", "")
