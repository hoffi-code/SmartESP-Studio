...
`lvgl` ist aus dieser Tabelle ausgeklammert: der Upstream-Dump listet dort alle ~250 LVGL-Style-/Root-Properties in einem einzigen Schema-Block (rekursiv, nicht pro Widget-Typ). Der LVGL-Builder ist schema-getrieben über `smartesp-studio-frontend/src/utils/lvglWidgets.js` — ein neues Widget = ein Schema-JSON unter `public/schemas/components/lvgl/widgets/` + eine Registry-Zeile. Abgedeckt: 24 von 29 Widget-Typen mit kuratierten Feldsätzen + Triggern (Runde 1: label/button/image + 12 Tier-1; Runde 2: buttonmatrix/meter/qrcode/spinner/animimg/tabview/tileview/keyboard/canvas). Feld-Keys, die eine kuratierte Schema nicht modelliert (z.B. `meter.scales`), bleiben beim Import→Export als `node.extra` verlustfrei erhalten. Jedes Widget-Schema `extends` `base_component/lvgl_widget_style.json` (kuratiertes ~20er-Style-Subset: `bg_color/opa`, `border_*`, `outline_*`, `radius`, `clip_corner`, `pad_*`, `text_color/font/align/opa`, `shadow_*`, `opa`), im Inspector als eigener „Style"-Abschnitt. Offen (voller Style-Satz, States/Parts, Flex/Grid, Top-Level-Optionen, Action-Katalog): siehe `plans/was-kommt-als-n-chstes-mossy-bonbon.md`.
