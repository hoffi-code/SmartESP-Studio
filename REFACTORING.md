...
11. **Offen:** `DisplayInspector.vue` Element-Typ-Split fortsetzen — Text/Image/Animation-Panels + `useDisplayFontControls`/`useDisplayImageField`, danach Graph/GraphLegend + `useGraphTraces`. Anders als bisher: Split der **Template**-Struktur in Kind-Komponenten, nicht nur Script-Extraktion — neue Art von Risiko (Props/Emits/Slots statt nur JS-Closures).
