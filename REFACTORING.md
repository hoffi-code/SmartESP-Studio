...
10. ✅ **DisplayInspector Seam #1 — Shape + Icon Panels:** `DisplayInspectorShape.vue` und `DisplayInspectorIcon.vue` — je ein eigenständiger Panel-Component mit eigenem `x/y/w/h`-Block (vorher geteilt über den `v-else`-Zweig), eigenem `color`-Picker-State und (bei Icon) der Aspect-Ratio-Lock-Logik aus `updateNumber`. Elternkomponente wählt jetzt per `v-if`/`v-else-if`/`v-else` zwischen Shape-Panel, Icon-Panel und dem verbliebenen Text/Image/Animation/Graph-Block. Nebenbei toten Code entsorgt: `showRotation`/`rotationOptions` im Elternteil waren nur im (nie erreichbaren) `v-else`-Zweig für Nicht-Shape-Typen verdrahtet. 10 neue Vitest-Component-Tests (`DisplayInspectorShape.spec.vue.js`, `DisplayInspectorIcon.spec.vue.js`). `DisplayInspector.vue`: 2114 → 1931 Z.
11. ✅ **DisplayInspector Seam #2 — Text + Image + Animation Panels:** `DisplayInspectorText.vue`, `DisplayInspectorImage.vue`, `DisplayInspectorAnimation.vue` + drei neue Composables in `src/composables/display/`: `useElementPatch` (generische `emit('update', …)`-Helfer, Aspect-Ratio-Lock optional per Flag statt hartkodierter Typliste), `useDisplayImageField` (Encoding-Status + Probe-Auto-Size, parametrisiert über `normalizeEncoding`/`typeKey` — von Image **und** Animation genutzt), `useDisplayFontControls` (Font-Source/-Variant-Logik, Patch-Feldnamen über einen `prefix`-Parameter generisch gehalten — `""` für Text, `"legendName"`/`"legendValue"` für die noch nicht extrahierten Graph-Legenden-Schriften). Die Legenden-Handler in `DisplayInspector.vue` selbst wurden auf dieselbe `useDisplayFontControls`-Instanz umgestellt (keine zweite Kopie von `visibleLocalFonts`/`deriveLocalStyle`/… mehr), obwohl Graph/Legend erst im nächsten Schritt als eigene Komponente rausgezogen wird. Nebenbei toten Code entsorgt: `updateNumber`s Aspect-Lock-Zweig und `updateBoolSelect`s `imageInvertAlpha`-Sonderfall im Elternteil waren nach dem Auszug von Image/Animation nicht mehr erreichbar. 20 neue Vitest-Component-Tests. `DisplayInspector.vue`: 1931 → 1108 Z.
12. ✅ **DisplayInspector Seam #3 — Graph + GraphLegend Panels:** `DisplayInspectorGraph.vue` (Sensor/Trace/Linie/Range-Felder, eigener `color`-Picker) + `DisplayInspectorGraphLegend.vue` (verschachtelt, nur bei `legendEnabled`) + `useGraphTraces` (Trace-CRUD + Per-Trace-Color-Picker-State). `DisplayInspectorGraph.vue` nutzt `useDisplayFontControls` nur für `buildDefaultFontDescriptor` (Default-Font beim ersten Legend-Einschalten), `DisplayInspectorGraphLegend.vue` für die eigentlichen Font-Source/-Family/-Variant-Handler — beide Komponenten instanziieren die Composable unabhängig (zustandslos, reine Prop-Ableitung, kein Teilen nötig). `DisplayInspector.vue` ist damit auf eine reine Dispatcher-Komponente geschrumpft (`v-if`/`v-else-if`-Kette über alle 6 Elementtypen), keine Element-Logik mehr im Elternteil. **Bug beim Umbau gefunden und behoben:** die layoutrelevanten `<style scoped>`-Regeln in `DisplayInspector.vue` (`.display-inspector__row(--quad)`, `.field-error(-text)`, `.display-trace-*`, `.display-legend`, `.display-icon-picker input`) griffen nach dem Auszug der Templates in Kind-Komponenten nicht mehr — Vue-Scoped-CSS matcht nur Elemente aus dem eigenen Template, nicht aus Kind-Komponenten. Alle betroffenen Selektoren auf `:deep(...)` umgestellt (Muster war für `.schema-icon-row`/`.schema-icon-btn` bereits vorhanden, jetzt konsequent für den Rest). Betraf rückwirkend auch die bereits committeten Shape/Icon- und Text/Image/Animation-Seams. 11 neue Vitest-Component-Tests. `DisplayInspector.vue`: 1108 → 314 Z. (gesamt 2114 → 314, Rest in 7 Panel-Komponenten + 4 Composables).
