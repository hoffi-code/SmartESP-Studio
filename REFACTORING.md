...
| **F1 / F5** | **In Arbeit.** `BuilderView.vue` (6293 → 4626 Z.), `DashboardView.vue` (3093 Z.), `DisplayInspector.vue` (2114 Z.) zerlegen. Siehe unten. |
...
5. Aus `BuilderView.vue` je ein weiteres Composable herausziehen, einzeln, mit eigener Suite: `useBuilderDeployment` (Deployment-State), `useBuilderProjectPersistence` (Projekt-Speichern/YAML/projects-index). Ziel Script-Setup < 1500 Z. — kein hartes Ziel, siehe Plan.
6. Doppelten `watch(config, {deep:true}) → saveConfig` (BuilderView + `useBuilderComponentCatalog.js:325`) konsolidieren.
7. `DashboardView.vue` / `DisplayInspector.vue` analog entlang funktionaler Schnitte.
