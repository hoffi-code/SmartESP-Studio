...
| **F1 / F5** | **In Arbeit.** `BuilderView.vue` (6293 → 4180 Z.), `DashboardView.vue` (3093 Z.), `DisplayInspector.vue` (2114 Z.) zerlegen. Siehe unten. |
...
6. Aus `BuilderView.vue` ein letztes Composable herausziehen: `useBuilderProjectPersistence` (Projekt-Speichern/YAML/projects-index). Ziel Script-Setup < 1500 Z. — kein hartes Ziel, siehe Plan.
7. Doppelten `watch(config, {deep:true}) → saveConfig` (BuilderView + `useBuilderComponentCatalog.js:325`) konsolidieren.
8. `DashboardView.vue` / `DisplayInspector.vue` analog entlang funktionaler Schnitte.
