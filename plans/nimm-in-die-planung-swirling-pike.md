...
| `Move Job/JobManager into ses/esphome` (`Job`, `JobManager`, `format_sse`, `job_manager`-Singleton). `_run_job` löst `_run_esphome` über `self`, `validate_host_serial_port` über `serial_ports.` auf. | M | ⬜ |
| `Split routes into ses/routes/* blueprints`, `bootstrap_storage()` → `create_app()`, test_yaml_import + test_smoke auf `from ses import create_app`, `server.py` → Dünn-Einstieg + `__main__` | L | ⬜ |
| `Add ses/ smoke coverage per blueprint` (ergänzt `test_smoke.py`) | S | ⬜ |
