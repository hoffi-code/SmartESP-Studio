# SmartESP Studio Docker Standalone

This folder contains Docker Compose examples for running SmartESP Studio as a standalone container. This is not a Home Assistant add-on and does not require Home Assistant Supervisor.
...
```text
SES_AUTH_MODE=basic
SES_AUTH_USERNAME=admin
SES_AUTH_PASSWORD=change-me
```

Change the password before use. If you set `SES_AUTH_MODE=none`, protect the app with another trusted layer.
...
Default project storage is `/config/smartesp`. Set `SES_USE_ESPHOME_SHARED_PATH=true` to use `/config/esphome`.
